...
try {
  ...
  await writeFile(path.join(toolProject, "package.json"), `${JSON.stringify({
    ...
    private: true,
    dependencies: {
      "@virtuecreation/composer-cli": `file:${cliTarball}`,
      "@virtuecreation/composer-registry": `file:${registryTarball}`,
    },
  }, null, 2)}\n`);
  await exec("npm", ["install", "--no-audit", "--no-fund"], { cwd: toolProject, maxBuffer: 10 * 1024 * 1024 });
