...
export default function FormSummary({ title, errors = [], status = errors.length ? "error" : "idle", message, focusOnMount = true, className }: FormSummaryProps) {
  ...
  return <div ref={ref} role={role} tabIndex={status === "error" ? -1 : undefined} className={className} data-vc-component="form-summary" data-vc-slot="root" data-vc-status={status}>
