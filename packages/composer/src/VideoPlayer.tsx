...
export default function VideoPlayer({ label, sources, tracks = [], fallback = "Your browser does not support this video.", className, controls = true, preload = "metadata", ...props }: VideoPlayerProps) {
  return <figure className={className} data-vc-component="video-player"><video {...props} aria-label={label} controls={controls} preload={preload} playsInline>{sources.map((source) => <source key={`${source.src}-${source.type}`} src={source.src} type={source.type} media={source.media} />)}{tracks.map((track) => <track key={`${track.src}-${track.kind}`} src={track.src} kind={track.kind} srcLang={track.srcLang} label={track.label} default={track.default} />)}{fallback}</video></figure>;
}
