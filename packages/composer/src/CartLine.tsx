...
export default function CartLine({ id, product, details, media, price, quantity, actions, status = "ready", error, className }: CartLineProps) {
  ...
  return <article className={className} aria-labelledby={headingId} aria-busy={status === "pending" || undefined} data-vc-component="cart-line" data-vc-status={status} data-vc-id={id}>
