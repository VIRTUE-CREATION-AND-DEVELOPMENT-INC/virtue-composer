...
export default function InlineMessage({ children, title, tone = "neutral", icon, live = tone === "danger" ? "assertive" : "polite", className }: InlineMessageProps) {
  return <div role={tone === "danger" ? "alert" : "status"} aria-live={live} className={className} data-vc-component="inline-message" data-vc-slot="root" data-vc-tone={tone}>{icon && <span aria-hidden="true" data-vc-inline-icon>{icon}</span>}<span>{title && <strong>{title}</strong>}{children}</span></div>;
}
