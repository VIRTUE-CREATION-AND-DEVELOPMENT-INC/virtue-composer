...
export default function ComboboxMultiSelect({ label, options, value, defaultValue = [], onValueChange, placeholder = "Select options", searchPlaceholder = "Search options", emptyText = "No options found", name, disabled, required, className }: ComboboxMultiSelectProps) {
  ...
  return <div className={className} data-vc-component="combobox-multi-select" data-vc-slot="root" data-vc-state={open ? "open" : "closed"}>
    <span data-vc-slot="label">{label}{required && <span aria-hidden="true"> *</span>}</span>
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild><button type="button" role="combobox" aria-label={label} aria-expanded={open} aria-required={required || undefined} disabled={disabled} data-vc-slot="trigger">{selected.length ? `${selected.length} selected` : placeholder}</button></Popover.Trigger>
      <Popover.Portal><Popover.Content align="start" data-vc-multi-content data-vc-slot="content"><Command data-vc-slot="command">
