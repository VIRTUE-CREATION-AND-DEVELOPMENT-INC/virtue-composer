...
export default function AudioPlayer({ label, sources, tracks = [], preload = "metadata", autoPlay, loop, muted, className }: AudioPlayerProps) {
  return <figure className={className} data-vc-component="audio-player">
    <figcaption>{label}</figcaption>
    <audio aria-label={label} controls preload={preload} autoPlay={autoPlay} loop={loop} muted={muted}>
      ...
      {sources.map((source) => <a key={source.src} href={source.src}>Download {label}</a>)}
    </audio>
  </figure>;
}
